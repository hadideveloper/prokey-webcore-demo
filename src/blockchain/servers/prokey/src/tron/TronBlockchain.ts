...
export class TronBlockchain extends ProkeyBaseBlockChain {
  ...
  public GetAddressInfo(
    reqAddresses: RequestAddressInfo | RequestAddressInfo[]
  ) {
  ...
  public async GetAccountInfo(
    ...
    try {
      let r = await this.GetFromServer<any>(
        `address/${this._coinName}/${account}`
      );
      if (!r.success) {
  ...
  public async GetAccountTransactions(
    account: string,
    limit: number = 10
  ): Promise<Array<TronTransactionDataInfo>> {
    let r = await this.GetFromServer<any>(
      `address/transactions/${this._coinName}/${account}/${limit}`
    );
    if (!r.success) {
  ...
  public async GetAccountTrc20Transactions(
    account: string,
    limit: number = 10
  ): Promise<Array<TronTrc20TransactionDataInfo>> {
    let r = await this.GetFromServer<any>(
      `address/trc20/transactions/${this._coinName}/${account}/${limit}`
    );
    if (!r.success) {
  ...
  public async GetNowBlock(): Promise<TronBlock> {
    return await this.GetFromServer<TronBlock>(
      `block/getblockcount/${this._coinName}`
    );
  }

  public async GetLatestBlock(count: number): Promise<any> {
    return await this.GetFromServer<TronBlock>(
      `block/GetLatest/${this._coinName}/${count}`
    );
  }

  public async BroadCastTransaction(data: string): Promise<any> {
    return await this.GetFromServer<any>(
      `Transaction/send/${this._coinName}/${data}`
    );
  }
}
